...
document
    ...
    .forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            document.getElementById('sequenceTitle')
                .textContent = id;
            document.getElementById('sequenceContent')
                .textContent =
                    fastaSequences[id] || 'Sequence not found';
            const modal = new bootstrap.Modal(
                document.getElementById('sequenceModal')
            );
            modal.show();
        });
    });
...
function createAlignmentCard(block, blockIndex) {
    ...
    card.innerHTML = `<div class="card-header small" id="${block.header.replace(/\|/g, '_')}"><span class="bg-dark text-info badge">${block.header}</span></div><div class="card-body"><div class="alignment-container"></div></div>`;
...
function finishSelection(event) {
    ...
    const editBtn = document.getElementById('editSelectionBtn'), insertBtn = document.getElementById('insertSelectionBtn');
...
function saveEditedSequence() {
    ...
    if (isInsertion) updateCoordinatesAfterInsertion(blockIndex, lineIndex, oldSequence, newSequence, startColumn, endColumn, sequenceLine);
    renderAlignments(alignmentBlocks);
    initializeTooltips();
    clearSelection();
...
function isInsertionSelection() {
    ...
    return selection.residues.length > 0 && selection.residues.every(r => r.dataset.residue === '-' || r.dataset.residue === '.');
}
